...
if __name__ == '__main__':
    soc_content = get_trade("socusdt")
    wicc_content = get_trade("wiccusdt")
    common_utils.write_json("get_trade", soc_content, map)

    # j_soc_content = json.dumps(soc_content, default = map)
    # j_wicc_content = json.dumps(wicc_content, default = map)

    soc_price = soc_content["tick"]["data"][0]["price"]
    wicc_price = wicc_content["tick"]["data"][0]["price"]
    delta_price = abs(soc_price - wicc_price / 10)
    percent = delta_price / soc_price
    print "soc = %s" \
          "; wicc = %s" \
          "; 当前soc和wicc的价格差为 %s" \
          "; 百分比为 %s" % (str(soc_price),
                         str(wicc_price),
                         str(delta_price),
                         str(percent))

    print json.dumps(soc_content, default=map)
    print json.dumps(wicc_content, default=map)

    # print get_symbols()
    # print get_kline("btcusdt", "5min", "10")